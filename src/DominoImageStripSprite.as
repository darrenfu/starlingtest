...
/**
	...
	public class DominoImageStripSprite extends StarlingImageStripSprite implements IInterpolatable {
		...
		public function DominoImageStripSprite(cacheKey:String = null, source:BitmapData = null, stripRect:Rectangle = null) {
...
//				}
		...
		override public function set index(value:int):void {
			...
			if (!mc) {
				mc = new MovieClip(currentAnimation);
				mc.stop();
				if (mRGBA >= 0) {
					mc.color = mRGBA >> 8;
					//TODO
					mc.alpha = .5;//(mRGBA & 0xFF) / 0xFF;
				}
				addChild(mc);
			}
		...
		override protected function generateTextures(withKey:String, fromBitmap:BitmapData = null):Vector.<Texture> {
			...
			var textures:Vector.<Texture> = null;//findCachedFrames(withKey);
			...
			if (!textures) {
				...
				var services:StarlingTableGameServices = StarlingTableGameServices.singleton();
				var raster:Texture = null;
				try {
//					if (services.hasRaster(withKey)) {
...
//					}
				...
				for (var frame:int = 0; frame < totalFrames; frame++) {
					...
					if (raster) {
//						textures.push(Texture.fromTexture(services.getRaster(withKey), bRect));
					} else {
