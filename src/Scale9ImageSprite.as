...
public class Scale9ImageSprite extends Scale9Image {
    ...
    protected var mPieces:Vector.<Vector.<uint>>;
    protected var mCorners:Vector.<Rectangle>;
    protected var mRaster:BitmapData;

    public function Scale9ImageSprite(raster:BitmapData, corners:Vector.<Rectangle>) {
        mRaster = raster;
        mCorners = corners;
        var textures:Scale9Textures = initTextures();
        super(textures, 1);
    }

    public function initTextures():Scale9Textures {
...
//        }
        ...
        mPieces = new Vector.<Vector.<uint>>(NUM_PIECES);
        for (var i:int = CORNER_NE; i <= SIDE_E; i++) {
//            var bytes:ByteArray = mRaster.getPixels(mCorners[i]);
//            bytes.position = 0;
//            mPieces[i] = bytes;
            var pixels:Vector.<uint> = mRaster.getVector(mCorners[i]);
            mPieces[i] = pixels;
        }
...
//        borderBmd.setVector(pieceRect, mPieces[CENTER]);
        ...
        borderBmd.setVector(pieceRect, mPieces[CORNER_NW]);
        ...
        borderBmd.setVector(pieceRect, mPieces[CORNER_NE]);
        ...
        borderBmd.setVector(pieceRect, mPieces[CORNER_SW]);
        ...
        borderBmd.setVector(pieceRect, mPieces[CORNER_SE]);
        ...
        borderBmd.setVector(pieceRect, mPieces[SIDE_N]);
        ...
        borderBmd.setVector(pieceRect, mPieces[SIDE_S]);
        ...
        borderBmd.setVector(pieceRect, mPieces[SIDE_W]);
        ...
        borderBmd.setVector(pieceRect, mPieces[SIDE_E]);
